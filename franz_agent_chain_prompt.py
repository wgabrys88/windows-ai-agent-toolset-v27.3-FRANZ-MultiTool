...
SYSTEM_PROMPT = """You are FRANZ, operating a Windows desktop using ONLY the provided tools.
...
- If your CHAIN_CONFIDENCE is < 0.65, output EXACTLY 1 tool call (a safe exploratory action).
...
FINAL STORY FORMAT (only in the FINAL tool call)
Write EXACTLY these lines (<= 1200 characters total):
[GOAL] ...
[STATE] ...
[LAST] ...
[NEXT] ...
...
TOOLS = [
    {
        ...
        "function": {
            "name": "click",
            "description": "Mouse: left click at normalized (x,y). story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            "name": "double_click",
            "description": "Mouse: double click at normalized (x,y). story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
                "properties": {"x": _xy_schema(), "y": _xy_schema(), "story": {"type": "string"}},
                "required": ["x", "y", "story"],
    ...
    },
    {
        ...
        "function": {
            "name": "right_click",
            "description": "Mouse: right click at normalized (x,y). story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
                "properties": {"x": _xy_schema(), "y": _xy_schema(), "story": {"type": "string"}},
                "required": ["x", "y", "story"],
    ...
    },
    {
        ...
        "function": {
            "name": "drag",
            "description": "Mouse: drag from (x1,y1) to (x2,y2) to move/paint. story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
                "properties": {
                    "x1": _xy_schema(),
    ...
    },
    {
        ...
        "function": {
            "name": "type_text",
            "description": "Keyboard: type the given text. story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            "name": "scroll",
            "description": "Mouse wheel: scroll by dy (positive=down, negative=up). story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            "name": "wait",
            "description": "No-op: wait for ms milliseconds (useful when tool calls are required). story may be empty for intermediate calls; only the final call should include the full HUD story.",
            "parameters": {
                "type": "object",
                ...
                "content": [
                    {
                        "type": "text",
                        "text": (
                            "Observe the screen and execute a CHAIN of 1 to 4 tool calls (ordered). If your CHAIN_CONFIDENCE is < 0.65, execute EXACTLY 1 tool call. For intermediate calls set story to an empty string; only the FINAL call includes the full HUD story in the required fixed format. Remember: do NOT add extra JSON keys; only the tool schema keys are allowed."
