...
def call_vlm(png: bytes) -> list[tuple[str, dict[str, Any]]]:
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": [{"type": "text", "text": "Analyze the screenshot and return tool_calls (1..N). Each tool call must include story."}, {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64.b64encode(png).decode('ascii')}"}}]},
        ],
        "tools": TOOLS,
