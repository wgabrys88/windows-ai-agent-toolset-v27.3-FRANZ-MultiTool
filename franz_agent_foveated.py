...
The cyan HUD window contains your narrative memory. Never click, type into, move, or close the HUD.

TIME MODEL
- The screenshot shows NOW (T).
- The HUD memory describes the most recent narrative (T-1).
...
[WHERE] Last action location and intent: tool + key coords + expected effect. (center / orientation)
...
DEFAULT_HUD_TEXT = (
    "[PAST] FRANZ is a desktop control agent inside a Windows 11 sandbox. The cyan HUD is his memory. "
...
class HUD:
    ...
    def _thread(self) -> None:
        ...
        hwnd = user32.CreateWindowExW(0, cls_name, "FRANZ HUD", style, x, y, w_, h_, None, None, hinst, None)
        ...
        # edit
        edit_style = WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FRANZ agent (multi-tool chain + foveated memory + last-action anchor)")
    ...
    print(f"Dump: {dump}")
    print("PAUSED - edit story in HUD if desired, click RESUME to start")
    ...
    with HUD() as hud:
        ...
        while not hud.stop.is_set():
            ...
            try:
                calls, raw_content = call_vlm(png, last_story, last_action, recent_actions, args.tool_choice)
